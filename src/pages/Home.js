...
class Home extends Component {
  ...
  addTodo = (todo) => {
    ...
    const existsTodo = this.state.todos.find(item => item === todo);
    const existsContent = this.state.todos.find(item => item.content === todo.content);
    console.log(existsTodo);
    console.log(existsContent);
    console.log(this.state.todos);

    if (existsContent !== undefined){
      return;
    } else {
      this.setState({
        todos: new_list,
